...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, fmt.Errorf(`unsupported time converting for string "%s"`, str)
	}
...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if len(fromZone) > 0 {
		if l, err = time.LoadLocation(fromZone[0]); err != nil {
			err = fmt.Errorf(`time.LoadLocation failed for name "%s"`, fromZone[0])
			return nil, err
		} else {
	...
	if l, err = time.LoadLocation(toZone); err != nil {
		err = fmt.Errorf(`time.LoadLocation failed for name "%s"`, toZone)
		return nil, err
	} else {
...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	if utils.IsNumeric(s) {
		...
		if err != nil {
			err = fmt.Errorf(`strconv.ParseInt failed for string "%s"`, s)
			return 0, err
		}
	...
	if len(match) == 3 {
		...
		if err != nil {
			err = fmt.Errorf(`strconv.ParseInt failed for string "%s"`, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = fmt.Errorf(`time.ParseDuration failed for string "%s"`, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
